...
function brentPoly(p, lb, ub, fa = brent_poly_Horner(p, lb), fb = brent_poly_Horner(p, ub)) {
    ...
    while (true) {
        ...
        const δ = 2 * brent_poly_u * max(1, abs(a), abs(b));
        const m = 0.5 * (c - b);
        //if (abs(m) <= δ || fb === 0) {
        if (abs(m) <= δ) {
...
function getInfos(hull) {
    ...
    for (let i = 0; i < len; i++) {
        ...
        info.ϕ = ϕ;
        const range = [get_infos_max(-Ω, ϕ - Ω), min(+Ω, ϕ + Ω)];
        info.range = range;
        // move ps to origin
...
function setShapeRotation(θ, cubics) {
    const sinθ = set_shape_rotation_sin(θ);
    const cosθ = set_shape_rotation_cos(θ);
